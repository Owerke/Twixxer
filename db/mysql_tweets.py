...
def get_tweets_for_user_by_username(username: str):
    ...
    try:
        ...
        cur = db.cursor()
        # Execute query (using the cursor)
        cur.execute("SELECT * FROM tweets WHERE username=%s ORDER BY created DESC;", (username,))
        # Fetch all data (from the cursor)
        results = cur.fetchall()
...
def create_tweet(tweet: Tweet):
    ...
    try:
        ...
        cur.execute("INSERT INTO tweets VALUES (%s, %s, %s, %s)", (tweet["id"], tweet["username"], tweet["content"], tweet["created"]))
